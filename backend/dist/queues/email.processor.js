    ...
    async process(job) {
        ...
        try {
            if (template === 'purchase-receipt') {
                ...
                await this.mailService.sendPurchaseReceipt(to, data?.orderId || 'UNKNOWN', data?.total || 0, data?.items || [], data?.subtotal || 0, data?.shippingCost || 0, data?.discountAmount || 0, data?.currency || 'NGN', data?.customerInfo);
                return;
            }
            ...
            <div class="info-section">
              ...
              <div class="additional-info">
                ...
                ${items.some((i) => i.customization?.embroideryName || i.customization?.specificInstructions) ?
                    items.filter((i) => i.customization?.embroideryName || i.customization?.specificInstructions).map((item) => `
